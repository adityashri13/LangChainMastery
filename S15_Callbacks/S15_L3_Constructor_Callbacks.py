...
# Set up chain with GPT-4.0 Mini and constructor callback
callbacks = [LoggingHandler()]  # Custom callback handler
llm = ChatOpenAI(model="gpt-4o-mini", callbacks=callbacks)  # Use GPT-4.0 mini model with constructor callback
...
# Run the chain (no need to pass callbacks explicitly in invoke)
